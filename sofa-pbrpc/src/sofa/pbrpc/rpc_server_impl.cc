    ...
    , _last_print_connection_ticks(0)
{
    ...
    _slice_count = max(1, 1000 / MAINTAIN_INTERVAL_IN_MS);
    _slice_quota_in = _options.max_throughput_in == -1 ?
        -1 : max(0L, _options.max_throughput_in * 1024L * 1024L) / _slice_count;
    _slice_quota_out = _options.max_throughput_out == -1 ?
        -1 : max(0L, _options.max_throughput_out * 1024L * 1024L) / _slice_count;
    _max_pending_buffer_size =
        max(0L, _options.max_pending_buffer_size * 1024L * 1024L);
    _keep_alive_ticks = _options.keep_alive_time == -1 ?
        -1 : max(1, _options.keep_alive_time) * _ticks_per_second;
...
void RpcServerImpl::OnAccepted(const RpcServerStreamPtr& stream)
{
    ...
    size_t read_base_block_factor = min(_options.read_buffer_base_block_factor, 
                                             (size_t)SOFA_PBRPC_TRAN_BUF_BLOCK_MAX_FACTOR);
    stream->set_read_buffer_base_block_factor(read_base_block_factor);

    size_t write_base_block_factor = min(_options.write_buffer_base_block_factor, 
                                              (size_t)SOFA_PBRPC_TRAN_BUF_BLOCK_MAX_FACTOR);
